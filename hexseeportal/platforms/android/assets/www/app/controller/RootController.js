HexseePortalApp.controller('RootController', function ($scope, $location, $rootScope, $timeout, $q, $routeParams) {
	...
	$scope.logoutMain = function () {
		...
		$timeout(function () {
			...
			ga('send', 'event', 'Logout', 'Click', HexseePortalApp.appData.userData.email);
		});
		...
		if (installationId == null) {
			...
			$rootScope.transferIO.removeDevice(installationId, function () {
				func_logout();
			});
		}
	...
	$rootScope.eventStartAdventure = function () {
		ga('send', 'event', 'Start a new Adventure', 'Click', HexseePortalApp.appData.userData.email);
	...
	*/
	...
	$rootScope.createPageIntoAdventure = function (adventure_id) {
		...
		$rootScope.transferIO.createPageIntoAdventure(adventure_id, appConfig.url_portal, document.title, function (data) {
			...
			console.log('createPageIntoAdventure'+JSON.stringify(data)+adventure_id);
	...
	$scope.clickReply = function (index, message_id) {
        ...
        console.log('click reply'+ message_id+ index);
	...
	$rootScope.SendBoxMessageDevice = function (message_id) {
		ga('send', 'event', 'Post a message', 'Click', HexseePortalApp.appData.userData.email);
	...
	$rootScope.submitSendBoxMessageDevice = function (message_id) {
        var bopChatPublic_el = $('.bopChatPublic');
		var _v = bopChatPublic_el.find('input').val();
		if (_v != '') {
			$rootScope.transferIO.post_message(message_id, _v, 'text', function (respone) {

				if (respone.message_id) {
                    bopChatPublic_el.find('input').val('');
                    bopChatPublic_el.fadeOut('slow', function () {
						$rootScope.boxReplyDevice = '';
					});
					//$timeout(function () {
					//	$rootScope.heightScrollTimeLineDeviceForCustomScrollBar(function () {
					//		$(".ms_scrollMessageTimelineDevice").mCustomScrollbar({
					//			advanced: {
					//				updateOnContentResize: true
					//			}
					//		});
					//		$('.ms_scrollMessageTimelineDevice').mCustomScrollbar("scrollTo", "bottom");
					//	});
					//}, 100);

				} else {
                    bopChatPublic_el.find('input').val('');
                    bopChatPublic_el.fadeOut('slow');
				}
			});
		}
        bopChatPublic_el.find('a').click(function () {
			$('.bopChatPublic').fadeOut('slow');
		})
	};
